...
- (void)reloadData
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        if (itemPerPage == 3) {
            ...
            float centerX = (1+column*2)*self.scrollView.frame.size.width/2;
...
-(id)init
{
    self = [super initWithFrame:CGRectMake(0, 0, 280, 70)];
    if (self) {
        ...
        self.titleBody = [[[UILabel alloc] initWithFrame:CGRectMake(70, 0, 210, 70)] autorelease];
        [titleBody setBackgroundColor:[UIColor clearColor]];
        [titleBody setTextAlignment:UITextAlignmentCenter];
        [titleBody setFont:[UIFont boldSystemFontOfSize:13]];
        [titleBody setTextColor:[UIColor whiteColor]];
        [titleBody setShadowColor:[UIColor blackColor]];
        [titleBody setShadowOffset:CGSizeMake(0, 0.5)];
        [titleBody setText:@""];
        [titleBody setTextAlignment:UITextAlignmentLeft];
        [titleBody setNumberOfLines:0];
        [self addSubview:titleBody];
    }
    return self;
}
